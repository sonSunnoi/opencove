...
export abstract class MultiLineLinkProvider implements ILinkProvider {
  ...
  provideLinks(bufferLineNumber: number, callback: (links: ILink[] | undefined) => void): void {
    ...
    const currentLine = linesWithOffsets.find(line => line.index === lineIndex)
    if (!currentLine) {
      callback(undefined)
      return
    }
    ...
    for (const match of combinedText.matchAll(regex)) {
      ...
      if (matchEnd <= currentLine.startOffset || matchIndex >= currentLine.endOffset) {
        continue
      }
  ...
  private offsetToPosition(
    ...
    for (const line of lines) {
      const isInLine = isEnd
        ...
        : offset < line.endOffset || (offset === line.startOffset && line.text.length === 0)
      if (!isInLine) {
        continue
      }

      const localOffset = Math.max(0, Math.min(offset - line.startOffset, line.text.length))
      return {
