...
import { type ContextLine, type LinkMatch, MultiLineLinkProvider } from './multi-line-link-provider'
...
export class UrlLinkProvider extends MultiLineLinkProvider {
  ...
  private createContextLine(index: number, text: string, leadingTrim = 0): ContextLine {
    return {
      index,
      lineNumber: index + 1,
      text,
  ...
  private getContinuationSegment(rawText: string): { leadingTrim: number; text: string } | null {
  ...
  private shouldAcceptContinuation(
    ...
    const continuationHasUrlSignal =
      URL_CONTINUATION_SIGNAL_PATTERN.test(continuationText) || /^[0-9]/.test(continuationText)
    const continuationAfterUrlBreak =
      URL_BREAK_SIGNAL_PATTERN.test(prevEnd) && /^[A-Za-z0-9]/.test(continuationText)
    const continuationLooksLikeWrappedWord =
      leadingTrim > 0 && /^[A-Za-z0-9]/.test(continuationText) && /[A-Za-z0-9]$/.test(trimmedPrev)
    const continuationLooksUrlLike =
      continuationHasUrlSignal || continuationAfterUrlBreak || continuationLooksLikeWrappedWord
    const continuationStartsHyphenToken = continuationText.startsWith('-')
    const hyphenTokenLooksUrlLike =
      continuationText.length > 1 &&
      (/[&/?#=.%_~]/.test(continuationText) || URL_BREAK_SIGNAL_PATTERN.test(prevEnd))
    const continuationStartsListMarker =
      prevEnd !== '-' && /^-(?:https?:\/\/|www\.)/i.test(continuationText)
    const prevIsBoundaryMarkerLine = this.isBoundaryMarkerLine(prevRawText.trimStart())
  ...
  private tryExtendForward(lines: ContextLine[]): boolean {
    ...
    if (this.getContextText(lines).length + continuation.text.length > MAX_HARD_WRAP_URL_LENGTH) {
      return false
    }

    if (!this.shouldAcceptContinuation(lastRawText, continuation.text, continuation.leadingTrim)) {
      return false
    }

    lines.push(this.createContextLine(last.index + 1, continuation.text, continuation.leadingTrim))
    return true
  }
  ...
  private tryExtendBackward(lines: ContextLine[]): boolean {
    ...
    if (this.getContextText(lines).length + prevRawText.length > MAX_HARD_WRAP_URL_LENGTH) {
      return false
    }
    if (
    ...
    if (!this.shouldAcceptContinuation(prevRawText, continuation.text, continuation.leadingTrim)) {
      return false
    }
  ...
  protected buildContextLines(lineIndex: number): ContextLine[] {
    ...
    while (backwardExtensions < MAX_HARD_WRAP_EXTENSION_LINES && this.tryExtendBackward(lines)) {
      backwardExtensions++
    }

    let forwardExtensions = 0
    while (forwardExtensions < MAX_HARD_WRAP_EXTENSION_LINES && this.tryExtendForward(lines)) {
      forwardExtensions++
    }
